function [] = par_sga(runNum, cell_number, protocol_number, isNormalized, scaleCaT, popsize, n_gen, n_stallgen)
    rng(runNum) % 'w' in main script
    addpath('Classes_all')
    addpath('dydts_all')
...
%     fitnessfcn = @sga_fitness_k19;
    fitnessfcn = @(x)sga_fitness_k19_parallel(x, runNum); 
    outputfcn = @(options,state,flag)ga_output_k19_parallel(options, state, flag, runNum) ;
    
    disp(['Making folder for rng ', int2str(runNum)])
    % Make sure another run isn't overridden
    if ~exist(['GA/Results/Run_', int2str(runNum)], 'dir')
        mkdir('GA/Results', ['Run_',int2str(runNum)]);
    else
        error(['Folder Run_',int2str(runNum),' already exists, please rename so previous runs are not overridden.'])
    end
    disp(['Finished making folder for rng ', int2str(runNum)])

    save('GA/curr_cell_protocol.mat', 'cell_number', 'protocol_number', 'isNormalized', 'scaleCaT') ;
    ...
    % lower and upper bounds
      ...
      %}
      %Boundaries for INaCa_fractionSS:

    %Initial population inside boundaries
    %initpop = (ub-lb)'.*lhsdesign(popsize,nvars) + lb'; %hypercube sampling
    initpop = 2*rand(popsize, nvars)-1; %uniform distribution from -1 to 1 --> multiplied to parameters as 2^x, or 0.5 to 2
    ...
    options = optimoptions(@ga, ...
        ...
        'OutputFcn', outputfcn, ... 
        'Display', 'iter',...
        'MaxGenerations', n_gen,... % 100
        'MaxStallGenerations', n_stallgen);   % if there is no change in the best fitness value for some number of generations, GA stops

    save(['GA/Results/Run_',int2str(runNum), '/Details.mat'], 'cell_number', 'protocol_number', 'isNormalized', 'options', 'popsize') ;
    ...
    save(['GA/Results/', 'Run_',int2str(runNum), '/minoptimparams.mat'], 'minoptimparams', 'optimparams') ;
    ...
    for j=1:length(protocol_number)
        ...
        %Make new plots and statistics to evaluate the results
        save(['GA/Results/', 'Run_',int2str(runNum), '/compare_results',int2str(i), '_', int2str(protocol_number(j))], ...
            'compare_AP', 'compare_CaT') ;
    end
