...
if (length(amplitudes) == length(numPulses) && length(amplitudes) == length(precedingTime))
    ...
    for i = 1:nPhases
        for j = 1:numPulses(i)
            ...
            if pulseIndex == 1 % can't use recursive method for first row
                intervalTimes(1, 1) = 0 ;
                intervalTimes(1, 2) = 0.2 ; % want a short delay at the beginning -> not enough for a spontaneous AP to squeeze in.
            else
    ...
    intervalTimes(2*pulseIndex-1, 2) = intervalTimes(2*pulseIndex-1, 1) + 1000/1000 ; % last row allows 1000ms for last beat
else
