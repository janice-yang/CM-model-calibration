function [normTimeToPlot,endTimeToPlot,normVmToPlot,endVmToPlot,normCaiToPlot,endCaiToPlot] = f_plot_lastSpontAPsAndCaTs(cells, drugTarget, percentBlock) % cells is a cell array of CMs
...
for i = 1:numcells
    ...
    if CM.state.t(end) < 2000 % assuming we aren't running this for <2 seconds or >2000s
    ...
    endTimeToPlot = [endTimeToPlot, timeinseconds(drug_start:drug_end)-timeinseconds(drug_start)];
    %endIndicesToPlot = length(endTimeToPlot)-1;
    endVmToPlot = [endVmToPlot, VminmV(drug_start:drug_end)];
    endCaiToPlot = [endCaiToPlot, CM.state.Y(drug_start:drug_end,CaiIndex)];
    ...
    normTimeToPlot = [normTimeToPlot, timeinseconds(norm_start:norm_end)-timeinseconds(norm_start)];
    %normIndicesToPlot = length(normTimeToPlot)-1;
    normVmToPlot = [normVmToPlot, VminmV(norm_start:norm_end)];
    normCaiToPlot = [normCaiToPlot, CM.state.Y(norm_start:norm_end,CaiIndex)];
...
%     legend('untreated', 'with drug')
    ...
    function [onet, oneV] = extractOne(multit, multiV)
        [~,dex] = min(multiV) ;
