function [dydt, ij_data] = f_ToRORd19(t, Y, celltype, universals, geometry, parameters, conductances, drugEffects, applicationTimes)
%% set universals
...
%% set conductances
for i = 1: length(conductances)
    if applicationTimes(1,i) <= t && applicationTimes(2,i) > t
        conductances(i) = conductances(i) .* drugEffects(i) ;
    end
end
INa_Multiplier = conductances(1) ; 
ICaL_Multiplier = conductances(2) ;
Ito_Multiplier = conductances(3) ;
INaL_Multiplier = conductances(4) ;
IKr_Multiplier = conductances(5) ;
IKs_Multiplier = conductances(6) ;
IK1_Multiplier = conductances(7) ;
IKb_Multiplier = conductances(8) ;
INaCa_Multiplier = conductances(9) ;
INaK_Multiplier = conductances(10) ;
